...
export const removeTaskPortfolioElementSchema = z
  .object({
    apiUrl: z
      .string()
      .url()
...
// inside removeTaskPortfolioElement() below, not via .superRefine() on the schema — chaining
...
export async function removeTaskPortfolioElement(
  client: TeamStormClient,
  params: z.infer<typeof removeTaskPortfolioElementSchema>
): Promise<{
  ...
  try {
    ...
    return {
      content: [
        {
          type: 'text',
          text: `✅ Задача ${taskId} откреплена от элемента портфеля \`${resolvedId}\``,
        },
      ],
      structuredContent: { taskId, portfolioElementId: resolvedId },
    };
  } catch (error) {
    ...
    return {
      content: [
        {
          type: 'text',
          text: `❌ Ошибка при откреплении задачи от элемента портфеля: ${error instanceof Error ? error.message : 'Неизвестная ошибка'}`,
        },
      ],
      isError: true,
    };
...
export function registerRemoveTaskPortfolioElementTool(server: McpServer, client: TeamStormClient) {
  server.registerTool(
    'teamstorm_remove_task_portfolio_element',
    {
      ...
      description:
        'Открепить (unpin) задачу от элемента портфеля, не затрагивая остальные закрепления задачи. Укажите portfolioElementId напрямую или portfolioElementName для автоматического поиска (при неоднозначности уточните portfolioId/folderId).',
      inputSchema: removeTaskPortfolioElementSchema,
      annotations: {
        readOnlyHint: false,
    ...
    async (params) => removeTaskPortfolioElement(client, params)
  );
}
