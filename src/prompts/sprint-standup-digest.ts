...
1. Вызови teamstorm_sprints_get (workspace="${workspace}", sprintId="${sprintId}"), чтобы получить название, даты и цель спринта.
2. Вызови teamstorm_tasks_list с фильтром по спринту sprintId="${sprintId}" (workspace="${workspace}"), чтобы получить задачи спринта.
3. Вызови teamstorm_tasks_list_updated (workspace="${workspace}"), чтобы понять, что менялось за последние сутки.

Сформируй дайджест из трёх разделов:
- ✅ Сделано со вчерашнего дня — задачи, перешедшие в статус категории "Done"/завершённые (сопоставь с изменениями из teamstorm_tasks_list_updated).
