...
1. Вызови teamstorm_tasks_list (workspace="${workspace}") с фильтрами по статусу и исполнителю согласно параметрам выше.
2. Если для приоритизации нужны кастомные поля (severity, priority и т.п.), для каждой задачи вызови teamstorm_attributes_get (workspace="${workspace}", taskId=<ID задачи>). Если кастомные поля не нужны — пропусти этот шаг.
