...
MCP-сервер для интеграции AI-агентов (Claude Code, Cursor) с TeamStorm API. Предоставляет **30 инструментов** для работы с задачами, папками, комментариями, атрибутами, вложениями, правами доступа, связями, пользователями, спринтами, workflow и списанием времени.
...
- **`src/tools/`** — 30 инструментов по доменам. Каждый: Zod-схема с `apiUrl` + `execute` → клиент → Markdown.
